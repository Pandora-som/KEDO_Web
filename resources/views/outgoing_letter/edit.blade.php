    ...
    <div class="incoming_letter_container">
        <form action="{{ route('outgoing_letter.update', $outgoingLetter->id) }}" method="post" class="form-div">
            ...
            <div class="incoming_letter_form">
                ...
                <input type="datetime-local" name="registration_date" id="registration_date" value="{{ $outgoingLetter->registration_date }}">
                ...
                <input type="text" name="incoming_number" id="incoming_number" value="{{ $outgoingLetter->incoming_number }}">

                <label for="classificator_id">Классификатор</label>
                <select name="classificator_id" id="classificator_id">
                    @foreach ($classificators as $classificator)
                        <option value="{{ $classificator->id }}" {{ $classificator->id === $outgoingLetter->classificator_id ? 'selected' : '' }}>{{ $classificator->classificator_name }}</option>
                    @endforeach
                </select></div>
